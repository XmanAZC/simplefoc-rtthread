...
void FOCMotor::useMonitoring(rt_device_t &print){
    ...
    for (size_t axis = 0; axis < 2; axis++)
    {
      for (size_t i = 0; i < iterations; i++)
      {
        ...
        for (size_t j = 0; j < cycles; j++)
        {
          ...
          t0 = _micros();
          _delay(risetime_us); // wait a little bit
          ...
          _delay(settle_us); // wait a bit for the currents to go to 0 again
...
void FOCMotor::monitor() {
  ...
  if(monitor_variables & _MON_TARGET){
    if(!printed && monitor_start_char) rt_kputs(&monitor_start_char);
    rt_kprintf("%f %d", target, monitor_decimals);   
    printed= true;
  }
  if(monitor_variables & _MON_VOLT_Q) {
    if(!printed && monitor_start_char) rt_kputs(&monitor_start_char);
    else if(printed) rt_kputs(&monitor_separator);
    rt_kprintf("%f %d", voltage.q, monitor_decimals);
    printed= true;
  }
  if(monitor_variables & _MON_VOLT_D) {
    if(!printed && monitor_start_char) rt_kputs(&monitor_start_char);
    else if(printed) rt_kputs(&monitor_separator);
    rt_kprintf("%f %d", voltage.d, monitor_decimals);
    printed= true;
  }
  ...
  if(monitor_variables & _MON_CURR_Q || monitor_variables & _MON_CURR_D) {
    ...
    if(monitor_variables & _MON_CURR_Q) {
      ...
      rt_kprintf("%f %d", c.q*1000, monitor_decimals); // mAmps
      printed= true;
    }
    if(monitor_variables & _MON_CURR_D) {
      if(!printed && monitor_start_char) rt_kputs(&monitor_start_char);
      else if(printed) rt_kputs(&monitor_separator);
      rt_kprintf("%f %d", c.d*1000, monitor_decimals); // mAmps
      printed= true;
    }
  }
  ...
  if(monitor_variables & _MON_VEL) {
    ...
    rt_kprintf("%f %d", shaft_velocity, monitor_decimals);
    printed= true;
  }
  if(monitor_variables & _MON_ANGLE) {
    if(!printed && monitor_start_char) rt_kputs(&monitor_start_char);
    else if(printed) rt_kputs(&monitor_separator);
    rt_kprintf("%f %d", shaft_angle, monitor_decimals);
    printed= true;
  }
  if(printed){
    if(monitor_end_char) rt_kputs(&monitor_end_char);
    else rt_kputs("\n");
  }
}   
