...
void Encoder::init(){
  ...
  if(pullup == Pullup::USE_INTERN){
    ...
    if(hasIndex()) rt_pin_mode(index_pin,PIN_MODE_INPUT_PULLUP);
  }else{
    rt_pin_mode(pinA, PIN_MODE_INPUT);
    rt_pin_mode(pinB, PIN_MODE_INPUT);
    if(hasIndex()) rt_pin_mode(index_pin, PIN_MODE_INPUT);
  }
...
void Encoder::enableInterrupts(void (*doA)(void *args), void(*doB)(void *args), void(*doIndex)(void *args)){
