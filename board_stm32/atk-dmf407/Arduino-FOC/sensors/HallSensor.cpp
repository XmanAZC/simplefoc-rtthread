...
// A channel
void HallSensor::handleA(void *args) {
  A_active= rt_pin_read(pinA);
  updateState();
}
// B channel
void HallSensor::handleB(void *args) {
  B_active = rt_pin_read(pinB);
  updateState();
}

// C channel
void HallSensor::handleC(void *args) {
  C_active = rt_pin_read(pinC);
  updateState();
}
...
void HallSensor::update() {
  ...
  if (use_interrupt) rt_hw_interrupt_enable(level);
...
void HallSensor::init(){
  ...
  if(pullup == Pullup::USE_INTERN){
    rt_pin_mode(pinA, PIN_MODE_INPUT_PULLUP);
    rt_pin_mode(pinB, PIN_MODE_INPUT_PULLUP);
    rt_pin_mode(pinC, PIN_MODE_INPUT_PULLUP);
  }else{
    rt_pin_mode(pinA, PIN_MODE_INPUT);
    rt_pin_mode(pinB, PIN_MODE_INPUT);
    rt_pin_mode(pinC, PIN_MODE_INPUT);
  }
...
void HallSensor::enableInterrupts(void (*doA)(void *args), void(*doB)(void *args), void(*doC)(void *args)){
  // attach interrupt if functions provided

  // A, B and C callback
  if(doA != nullptr)
  {
    rt_pin_attach_irq(pinA, 
                      PIN_IRQ_MODE_RISING | PIN_IRQ_MODE_FALLING, 
                      doA, 
                      RT_NULL);
  }
  if(doB != nullptr)
  {
    rt_pin_attach_irq(pinB, 
                      PIN_IRQ_MODE_RISING | PIN_IRQ_MODE_FALLING, 
                      doB, 
                      RT_NULL);
  }
